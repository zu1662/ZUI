...
const watcher = chokidar.watch(path.resolve(__dirname, '../src/components'), {
  ignored: /(^|[\/\\])\../
})
watcher
  .on('addDir', (dirpath) => {
    let dirName = dirpath.split(path.sep).pop()
    if(dirName === 'New Folder' || dirName === 'components') return
...
function updateFiles() {
  ...
  let listTemplate = []
  // 获取目录下的所有组件文件夹名称
  var dirs = fs.readdirSync(path.resolve(__dirname, '../src/components'));
