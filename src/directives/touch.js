 ...
 * @LastEditTime: 2019-08-08 23:19:03
 * @Description: 触摸滑动指令。可以判断当前滑动方向
 */
const vueTouch = {
  distance: 30,
  duration: 300
...
const touch = {
  bind (el, binding) {
     ...
     */
    const distance = binding.arg.distance || vueTouch.distance
    const duration = binding.arg.distance || vueTouch.duration
    const triggerOnMove = binding.arg.triggerOnMove
    const moving = binding.arg.moving
    const isStopPropagation = binding.arg.isStopPropagation
    const isPreventDefault = binding.arg.isPreventDefault
    ...
    el.addEventListener('touchmove', e => {
      ...
      if (startPoint) {
        ...
        if (triggerOnMove) {
          // 判断此时的滑动方向
          const nowDirection = swipeDirection(
            startPoint.x,
            startPoint.y,
            endPoint.x,
            endPoint.y
